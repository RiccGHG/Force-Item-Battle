/**
Third-party code
Source: https://github.com/TaranGauri/GalactiXDB
...
export class itemDB {
  ...
  constructor(namespace = "", cacheSize = 100, saveRate = 1) {
    system.run(() => {
      ...
      function startLog() {
        placeHolder(
          `§qitemDB > is initialized successfully.§r namespace: ${self.#settings.namespace} §r${date()} `,
        );
      }
      const VALID_NAMESPACE_ERROR = new Error(
        `§citemDB > ${namespace} isn't a valid namespace. accepted char: A-Z a-z 0-9 _ §r${date()}`,
      );
      ...
      if (player)
        if (!sl || sl?.hasParticipant("x") === false) {
          ...
          this.#dimension.runCommand(
            `/tickingarea add ${this.#sL.x} 319 ${this.#sL.z} ${this.#sL.x} 318 ${this.#sL.z} storagearea`,
          );
          startLog();
        } else {
          this.#sL = { x: sl.getScore("x"), y: 318, z: sl.getScore("z") };
      ...
      function log() {
        const abc =
          (-(self.#queuedKeys.length - lastam) / 6).toFixed(0) || "//";
        self.logs.save == true;
      ...
      world.beforeEvents.playerLeave.subscribe(() => {
        if (this.#queuedKeys.length && world.getPlayers().length < 2) {
          console.error(
            `\n\n\n\n§citemDB > Fatal Error > World closed too early, items not saved correctly.  \n\n` +
  ...
  #load(key) {
    if (key.length > 30) {
      throw new Error(
        `§citemDB > Out of range: <${key}> has more than 30 characters §r${date()}`,
      );
    }

    let canStr = false;
    try {
      world.structureManager.place(key, this.#dimension, this.#sL, {
    ...
    if (!entity)
      throw new Error(
        `§citemDB > Failed to create or find gxdb:storage entity`,
      );

    const invComp = entity.getComponent("inventory");
    if (!invComp)
      throw new Error(
        `§citemDB > Storage entity missing inventory component`,
      );

    const inv = invComp.container;

    if (this.logs.load) {
      placeHolder(`§citemDB > Loaded inventory for key ${key}`);
    }
    return { canStr, inv };
  }
  ...
  async #romSave(key, value) {
    ...
    if (Array.isArray(value)) {
      try {
        ...
        throw new Error(
          `§cQIDB > Invalid value type. supported: ItemStack | ItemStack[] | undefined §r${date()}`,
        );
      }
  ...
  set(key, value) {
    if (!this.#validNamespace)
      throw new Error(
        `§cQitemDB > Invalid name: <${this.#settings.namespace}>. accepted char: A-Z a-z 0-9 _ §r${date()}`,
      );
    if (!/^[A-Za-z0-9_]*$/.test(key))
      throw new Error(
        `§citemDB > Invalid name: <${key}>. accepted char: A-Z a-z 0-9 _ §r${date()}`,
      );
    const time = Date.now();
    ...
    if (Array.isArray(value)) {
      if (value.length > 255)
        throw new Error(
          `§citemDB > Out of range: <${key}> has more than 255 ItemStacks §r${date()}`,
        );
      world.setDynamicProperty(key, true);
    } else {
  ...
  get(key) {
    if (!this.#validNamespace)
      throw new Error(
        `§citemDB > Invalid name: <${this.#settings.namespace}>. accepted char: A-Z a-z 0-9 _ §r${date()}`,
      );
    if (!/^[A-Za-z0-9_]*$/.test(key))
      throw new Error(
        `§citemDB > Invalid name: <${key}>. accepted char: A-Z a-z 0-9 _ §r${date()}`,
    ...
    if (this.#quickAccess.has(key)) {
      this.logs.get == true &&
        placeHolder(
          `§aitemDB > Got key <${key}> succesfully. ${Date.now() - time}ms §r${date()}`,
        );
      return this.#quickAccess.get(key);
    }
    const structure = world.structureManager.get(key);
    if (!structure)
      throw new Error(`§citemDB > The key < ${key} > doesn't exist.`);
    const { canStr, inv } = this.#load(key);
    const items = [];
    for (let i = 0; i < 256; i++) items.push(inv.getItem(i));
    for (let i = 255; i >= 0; i--)
      if (!items[i]) items.pop();
      else break;
    this.#save(key, canStr);
    this.logs.get == true &&
      placeHolder(
        `§aitemDB > Got items from <${key}> succesfully. ${Date.now() - time}ms §r${date()}`,
      );
  ...
  has(key) {
    ...
    this.logs.has == true &&
      placeHolder(
        `§aitemDB > Found key <${key}> succesfully. ${Date.now() - time}ms §r${date()}`,
      );
  ...
  delete(key) {
    ...
    if (structure)
      ...
      throw new Error(
        `§citemDB > The key <${key}> doesn't exist. §r${date()}`,
      );
    this.logs.delete == true &&
      placeHolder(
        `§aitemDB > Deleted key <${key}> succesfully. ${Date.now() - time}ms §r${date()}`,
      );
  }
  ...
  keys() {
    ...
    allIds
      ...
      .forEach((id) =>
        ids.push(id.replace(this.#settings.namespace + ":", "")),
      );
    this.logs.keys == true &&
      placeHolder(
        `§aitemDB > Got the list of all the ${ids.length} keys. §r${date()}`,
      );
  ...
  values() {
    ...
    this.logs.values == true &&
      placeHolder(
        `§aitemDB > Got the list of all the ${values.length} values. ${Date.now() - time}ms §r${date()}`,
      );
  ...
  clear() {
    ...
    this.logs.clear == true &&
      placeHolder(
        `§aitemDB > Cleared, deleted ${filtered.length} values. ${Date.now() - time}ms §r${date()}`,
      );
  }
}
